...
public class Username {
        ...
        while (!input[0].equals("Sign")) {
            ...
            switch (command) {
                ...
                case "Reverse":
                    ...
                    if (isIndexValid(startIndex, endIndex, username)) {
    ...
    private static boolean isSubstringExist(StringBuilder username, String substring) {
        String check = username.toString();
        return check.contains(substring);
    }

    private static boolean isIndexValid(int startIndex, int endIndex, StringBuilder username) {
        return startIndex >= 0 && startIndex <= username.length() - 1 && endIndex >= 0 && endIndex <= username.length();
    }
}
